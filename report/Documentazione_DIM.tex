...
\textbf{Responsabilità:} OpenTelemetry Collector raccoglie log strutturati da n8n via OTLP e log da tutti gli altri container Docker via \texttt{receiver\_creator} con \texttt{docker\_observer}. Per le metriche, esegue scraping Prometheus sia del Redis Exporter (metriche Redis) sia dell'endpoint \texttt{/metrics} nativo di n8n (workflow attivi, memoria heap/RSS, event loop lag, handle attivi). I segnali vengono esportati verso Loki (log) e Prometheus (metriche). Grafana visualizza i due segnali con datasource preconfigurati.
...
n8n è configurato per emettere telemetria (log e metriche OTLP) verso l'OpenTelemetry Collector e per esporre metriche applicative Prometheus:
...
\textbf{Nota:} \texttt{N8N\_METRICS=true} abilita l'endpoint Prometheus su \texttt{:5678/metrics}, che espone metriche applicative come il conteggio dei workflow attivi, la memoria heap Node.js, la memoria RSS del processo, il lag dell'event loop e gli handle attivi. \texttt{N8N\_METRICS\_INCLUDE\_DEFAULT\_METRICS=true} include le metriche di sistema Node.js.
...
\midrule
otlp (HTTP) & Log, Metriche & Riceve telemetria nativa strutturata da n8n sulla porta 4318 \\
...
prometheus (scrape) & Metriche & Fa scraping di \texttt{redis-exporter:9121/metrics} (metriche Redis) e \texttt{n8n:5678/metrics} (metriche applicative n8n: workflow attivi, heap, RSS, event loop) \\
\bottomrule
\end{tabularx}
...
Prometheus (v2.53.0) è il backend per le metriche. Non esegue scraping diretto dei target: le metriche arrivano esclusivamente via Remote Write dall'OTel Collector, che a sua volta fa scraping del Redis Exporter e dell'endpoint nativo di n8n.

\textbf{Metriche disponibili:}
\begin{itemize}
  ...
  \item \textbf{n8n} (via endpoint \texttt{/metrics}): \texttt{n8n\_active\_workflow\_count}, \texttt{n8n\_nodejs\_heap\_size\_used\_bytes}, \texttt{n8n\_process\_resident\_memory\_bytes}, \texttt{n8n\_nodejs\_eventloop\_lag\_seconds}, \texttt{n8n\_nodejs\_active\_handles\_total}.
\end{itemize}
