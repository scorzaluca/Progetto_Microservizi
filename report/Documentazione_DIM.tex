...
\begin{itemize}
  ...
  \item \textbf{Osservabilità Completa:} Monitoraggio end-to-end di log, tracce distribuite e metriche per tutti i componenti del sistema.
\end{itemize}
...
\begin{itemize}
  \item \textbf{Osservabilità:} Raccolta centralizzata di log (Loki), tracce distribuite (Tempo) e metriche (Prometheus) tramite OpenTelemetry Collector, con dashboard Grafana preconfigurate.
...
\textbf{Responsabilità:} OpenTelemetry Collector raccoglie log da tutti i container Docker via \texttt{receiver\_creator} con \texttt{docker\_observer}, riceve telemetria OTLP da n8n, e fa scraping delle metriche Redis. I segnali vengono esportati verso Loki (log), Tempo (tracce) e Prometheus (metriche). Grafana visualizza i tre segnali con datasource preconfigurati e correlazione automatica log-tracce.
...
% --- Placeholder per immagine architettura microservizi ---
\begin{figure}[H]
\centering
% \includegraphics[width=\textwidth]{img/architettura_microservizi.png}
\fbox{\parbox{0.9\textwidth}{\centering\vspace{3cm}\textit{Inserire qui l'immagine dell'architettura a microservizi completa}\vspace{3cm}}}
...
\begin{itemize}
  ...
  \item \textbf{Il Supervisore (Stack Osservabilità):} Pipeline completa OpenTelemetry Collector $\to$ Loki/Tempo/Prometheus $\to$ Grafana che raccoglie e correla log, tracce distribuite e metriche da tutti i container.
\end{itemize}
...
tempo         & grafana/tempo:2.9.0         & 3200         & \texttt{./tempo/tempo-config.yml} \\
...
\bottomrule
\end{tabularx}
\caption{Dettaglio dei servizi Docker Compose}
\end{table}
...
\begin{itemize}
  \item \textbf{n8n} dipende da \texttt{redis} e \texttt{ollama} per operare correttamente.
  \item \textbf{otel-collector} accede al Docker socket (\texttt{/var/run/docker.sock}) per la discovery automatica dei container e la raccolta dei log tramite \texttt{receiver\_creator}.
  \item \textbf{redis-exporter} si connette a \texttt{redis:6379} per esporre le metriche in formato Prometheus.
  \item \textbf{grafana} è preconfigurato con datasource per Loki, Tempo e Prometheus tramite il provisioning automatico in \texttt{grafana/provisioning/datasources/datasources.yml}.
\end{itemize}
...
\begin{technicalbox}{Dettaglio Tecnico: Telemetria OTLP di n8n}
n8n supporta nativamente l'esportazione di tracce e log via protocollo OTLP verso un OpenTelemetry Collector. La configurazione \texttt{N8N\_OTEL\_SPAN\_SAMPLING\_RATE=1} garantisce il campionamento del 100\% delle tracce, essenziale in ambiente di development per avere visibilità completa su ogni esecuzione di workflow.
\end{technicalbox}
...
\begin{itemize}
  ...
  \item \textbf{Meccanismo di Polling:} Dopo il completamento della Fase~5 (rilevato dal tag \texttt{[FASE\_5\_COMPLETA]} nella risposta), il frontend avvia un polling periodico verso il Polling Workflow per attendere la generazione del modello architetturale.
  ...
  \item \textbf{Indicatore di Fase:} Un indicatore visivo nella parte superiore della chat mostra la fase corrente dell'intervista (1--5 o ``Modello Generato'').
\end{itemize}
...
% --- Placeholder per immagine architettura osservabilità ---
\begin{figure}[H]
\centering
\fbox{\parbox{0.9\textwidth}{\centering\vspace{3cm}\textit{Inserire qui l'immagine dell'architettura dello stack di osservabilità}\vspace{3cm}}}
\caption{Architettura dello stack di osservabilità: OTel Collector $\to$ Loki / Tempo / Prometheus $\to$ Grafana}
...
receiver\_creator\newline con filelog & Log & Scopre automaticamente i container Docker tramite \texttt{docker\_observer} e raccoglie i log di ogni container \\
...
otlphttp/tempo & Tempo (4318) & Esporta tracce distribuite via OTLP HTTP \\
...
\begin{technicalbox}{Dettaglio Tecnico: receiver\_creator e docker\_observer}
L'extension \texttt{docker\_observer} si connette al Docker socket (\texttt{unix:///var/run/docker.sock}) e monitora lo stato dei container in tempo reale. Per ogni container scoperto, il \texttt{receiver\_creator} istanzia dinamicamente un receiver \texttt{filelog} che legge i log del container dal filesystem dell'host. Questo approccio garantisce la raccolta automatica dei log di tutti i container senza necessità di configurazione manuale per ogni nuovo servizio aggiunto allo stack.

La configurazione utilizza un operatore \texttt{container} nel parser filelog che estrae automaticamente metadati come \texttt{container\_id}, \texttt{container\_name} e il formato del log (JSON, CRI, Docker).
\end{technicalbox}
...
\section{Tempo: Tracce Distribuite}
...
\textbf{Configurazione chiave:}
\begin{itemize}
  \item Receiver OTLP su HTTP (4318) e gRPC (4317).
  \item Storage locale con blocchi compressi in \texttt{/var/tempo/traces}.
  \item SLO configurato a 5 secondi per ricerca tracce e lookup per TraceID.
\end{itemize}
...
\section{Grafana: Dashboard e Correlazione}
...
\toprule
\textbf{Datasource} & \textbf{Backend} & \textbf{Default} & \textbf{Funzionalità Speciale} \\
\midrule
Loki       & \texttt{loki:3100}       & Sì & Derived fields per correlazione log $\to$ tracce via \texttt{trace\_id} \\
Tempo      & \texttt{tempo:3200}      & No & TraceQL, correlazione tracce $\to$ log via Loki \\
...
\begin{figure}[H]
\centering
\fbox{\parbox{0.9\textwidth}{\centering\vspace{2.5cm}\textit{Inserire qui screenshot della vista tracce in Tempo}\vspace{2.5cm}}}
\caption{Grafana Tempo --- Visualizzazione tracce distribuite di una esecuzione workflow n8n}
\label{fig:grafana_tempo}
\end{figure}

\begin{figure}[H]
\centering
\fbox{\parbox{0.9\textwidth}{\centering\vspace{2.5cm}\textit{Inserire qui screenshot della correlazione log-tracce}\vspace{2.5cm}}}
\caption{Correlazione log $\to$ tracce in Grafana tramite \texttt{trace\_id}}
\label{fig:grafana_correlazione}
\end{figure}

\begin{technicalbox}{Dettaglio Tecnico: Correlazione Log--Tracce}
La correlazione tra log e tracce è implementata tramite \textit{derived fields} nel datasource Loki. Il campo \texttt{trace\_id} presente nei log JSON di n8n viene estratto tramite la regex \texttt{`"trace\_id":"(\textbackslash w+)"`} e linkato al datasource Tempo. In Grafana, questo si traduce in un link cliccabile accanto a ogni riga di log che contiene un \texttt{trace\_id}, permettendo di navigare direttamente alla traccia corrispondente per analizzare l'intera esecuzione del workflow.
\end{technicalbox}
...
\begin{itemize}
  ...
  \item \textbf{Osservabilità completa:} Stack OpenTelemetry end-to-end con correlazione automatica log-tracce che garantisce visibilità totale sull'esecuzione del sistema.
