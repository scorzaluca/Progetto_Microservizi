...
\textbf{Problema:} Una sessione di discovery completa può generare centinaia di messaggi attraverso cinque fasi. Se tutti i messaggi venissero inclusi nel contesto LLM degli agenti delle fasi successive, il context window sarebbe saturato rapidamente, causando degradazione della qualità delle risposte o errori di troncamento.
...
\textbf{Problema:} La catena DDD Analyst $\to$ JSON Coder con modello \texttt{qwen2.5:14b} in inferenza locale può richiedere 5-15 minuti su hardware consumer (CPU-only o GPU entry-level).
...
\textbf{Trade-off:} L'esperienza utente durante la generazione è migliorata dall'animazione, ma il tempo di attesa rimane significativo. Un deployment su GPU dedicata (es. NVIDIA RTX 3090+) ridurrebbe la latenza di un ordine di grandezza.
...
\textbf{Soluzione:} Il Project Controller include una regola esplicita nel suo system prompt: se l'utente è evasivo o delega la decisione, emette \texttt{WAIT} e l'agente della fase corrente continua a porre domande, forzando lo stakeholder a fornire informazioni concrete.
...
\begin{enumerate}
  ...
  \item \textbf{Assenza di Persistenza Long-Term:} Le chiavi Redis hanno un TTL di 24 ore. Una sessione non ripresa entro quel termine viene persa. Non esiste un meccanismo di export/import delle sessioni.
\end{enumerate}

\subsection{Limiti Legati all'Inferenza Locale}

\begin{enumerate}
  \item \textbf{Velocità:} L'inferenza locale su CPU è ordini di grandezza più lenta rispetto alle API cloud. Una singola risposta dell'agente può richiedere 10-30 secondi.
  ...
  \item \textbf{Requisiti Hardware:} Ollama con \texttt{qwen2.5:14b} richiede almeno 16 GB di RAM (o GPU con 10+ GB VRAM) per operare senza swap su disco.
\end{enumerate}
...
\addlinespace
n8n vs. codice custom & Sviluppo rapido, debug visuale, nessun codice da mantenere & Vendor lock-in su n8n, meno flessibilità per logiche complesse, overhead HTTP tra sub-workflow \\
\addlinespace
Frontend monolitico HTML vs. React/Vue & Zero dipendenze, zero build step, deploy immediato & Mancanza di componenti riutilizzabili, stato UI gestito con variabili globali \\
\addlinespace
...
\addlinespace
Redis vs. database relazionale & Latenza sub-millisecondo, TTL nativo, semplicità & Nessuna query relazionale, nessuno schema enforced, rischio di perdita dati senza persistenza RDB/AOF \\
\addlinespace
