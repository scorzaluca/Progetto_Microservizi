...
\textit{Inserire qui un diagramma architetturale completo del sistema (simile ad un C4 Context/Container Diagram) che mostri tutti i microservizi, le rete Docker, i flussi di comunicazione e lo stack di osservabilità. Questo è il diagramma più importante dell'intero report.}
\end{warningbox}
...
\begin{itemize}
  ...
  \begin{itemize}
    \item \texttt{llama3:latest} --- modello conversazionale impiegato per gli agenti che interagiscono direttamente con l'utente o ne valutano le risposte: Interviewer (temp.~0.4), Orchestrator (temp.~0.0), Modifier (temp.~0.1) ed Explainer (temp.~0.4).
    \item \texttt{qwen2.5:14b} --- modello ad alta precisione strutturale, impiegato dove è richiesta la generazione di output formali: DDD Analyst per la sintesi del documento architetturale (temp.~0.3) e JSON Coder per la traduzione in JSON con schema predefinito (temp.~0.0).
  \end{itemize}
  ...
  \item \textbf{Il Supervisore (Stack Osservabilità):} Pipeline OpenTelemetry Collector $\to$ Loki/Tempo/Prometheus $\to$ Grafana che raccoglie log, tracce distribuite e metriche da tutti i container.
\end{itemize}
...
L'osservabilità non è un add-on ma un componente strutturale dell'architettura. Il servizio n8n emette telemetria nativa OTLP verso l'OTel Collector, che la instrada verso Loki (log), Tempo (tracce) e Prometheus (metriche). Grafana è preconfigurato con datasource e correlation per navigare tra i tre segnali.
...
\textbf{Modelli LLM --- llama3 e qwen2.5:14b:} \texttt{llama3} è impiegato per tutti gli agenti conversazionali (intervistatori, orchestratore, Modifier, Explainer) dove fluency e capacità di interazione sono prioritarie. \texttt{qwen2.5:14b} è riservato agli agenti di elaborazione post-intervista --- DDD Analyst e JSON Coder --- che richiedono alta precisione nella generazione di output strutturati (documento Markdown e JSON formale).
\end{technicalbox}

\section{Flusso di Comunicazione End-to-End}

Il flusso di una sessione completa segue questo percorso, dal primo messaggio dell'utente fino al raffinamento del modello:
...
% ── NOTA IMMAGINE ────────────────────────────────────────────────────────────
\begin{warningbox}{Suggerimento: Immagine}
\textit{Inserire qui un diagramma di sequenza (sequence diagram) che illustri il flusso completo descritto sopra: Frontend $\to$ Routing $\to$ Discovery $\to$ Generazione $\to$ Refinement. Evidenziare il ruolo dell'Orchestrator nella validazione e la catena di generazione post-Fase~5.}
\end{warningbox}
